...
// Package ssl manages certificate authorities. A CA's private key is encrypted
// at rest with the KEK; the public certificate is stored and served in clear.
// Leaf certificates are issued on demand and returned, not persisted.
package ssl
...
// Issue signs a leaf certificate with the named CA and returns the certificate
...
func (s *Service) Issue(caUID, commonName string, dnsNames []string, validDays int) (certPEM, keyPEM []byte, err error) {
	...
	ca, err := pki.LoadCA(res.Status.CertPEM, keyPEMRaw)
	if err != nil {
		return nil, nil, err
	}
	if validDays == 0 {
		validDays = defaultCertDays
	}
